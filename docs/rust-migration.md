...
- Normalize supported mid-conversation system instructions into the single leading Chat Completions `system` message. Preserve assistant prefills. Reject unsupported roles, content blocks, cache-only requests, Anthropic server tools, and server-tool state instead of silently dropping or approximating them.
