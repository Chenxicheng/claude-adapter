...
- Match the TypeScript/OpenAI SDK wire contract for required transport fields: send `Accept: application/json`, `Content-Type: application/json`, bearer authorization, and an explicit `stream` boolean. Keep the adapter's truthful user agent; provider-specific SDK telemetry headers are not part of the API contract.
...
- Normalize supported mid-conversation system instructions into the single leading Chat Completions `system` message. Skip contentless hook messages and filter the short assistant-prefill tokens that the TypeScript adapter filtered for OpenAI-compatible providers. Reject unsupported roles, content blocks, cache-only requests, Anthropic server tools, and server-tool state instead of silently dropping or approximating them.
- Tool messages remain text-only. Emit tool messages first, then one user image message whose text labels identify each resolved `tool_call_id`, then any ordinary user content. Validate tool names, choices, result ordering, and result-to-call associations before forwarding.
- Map all four Anthropic client `tool_choice` modes and omit no-op defaults. Validate `disable_parallel_tool_use` and explicit `strict: true`, but omit them from the upstream request to retain the established TypeScript wire shape accepted by strict third-party providers. Accept `cache_control` without conversion; provider-specific cache behavior belongs to the configured upstream.
- Use `max_completion_tokens` only for GPT-5 and OpenAI o-series models and `max_tokens` elsewhere; retain the Azure minimum-token adjustment. Validate but do not forward `metadata.user_id`. Map `output_config.effort` only for recognized OpenAI reasoning models and preserve existing GLM/Qwen model-family behavior, including tool-turn reasoning history. Validate `output_config.format`, but omit it from the upstream request for the same wire-compatibility boundary.
- Omit empty stop sequences and reject stop sequences for known OpenAI reasoning models that do not support them. Reject unknown request fields and generic thinking modes that cannot be represented exactly. `top_k` remains unsupported.
