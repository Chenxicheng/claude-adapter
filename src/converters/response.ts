...
import { buildAnthropicUsageFromOpenAI } from './usage';
...
export function convertResponseToAnthropic(
  ...
  // Build usage
  const usage: AnthropicUsage = buildAnthropicUsageFromOpenAI(openaiResponse.usage);
...
function getReasoningContent(
  message: OpenAIChatResponse['choices'][number]['message'] | undefined
): string {
  if (!message) {
    return '';
  }
